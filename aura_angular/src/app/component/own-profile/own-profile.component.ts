...
export class OwnProfileComponent implements OnInit {
  ...
  // Function to get the liked products from the user
        ...
        for (let index = 0; index < this.allLikeProducts.length; index++) {
          if (this.allLikeProducts[index].id_user == this.id_selected_user) {
            ...
            for (let index = 0; index < prod.length; index++) {
              if (prod[index].id == id_product) {
                this.user_fav_products.push(prod[index]);
              }
            }
          }
        }
  ...
  // Function to get the liked users from the user
